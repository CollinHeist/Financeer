...
import { getIncomeTransactions } from '@/lib/api';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function IncomeTransactionSummary({
    isOpen,
    onOpenChange,
    incomeId,
    incomeName,
  ...
  const { data: transactions, isLoading, error } = useQuery({
    queryKey: ['incomeTransactions', incomeId],
    queryFn: () => getIncomeTransactions(incomeId),
  ...
  const aggregateTransactions = (transactions, frequency) => {
    ...
    const groupedData = transactions.reduce((acc, transaction) => {
      ...
      switch (frequency.unit) {
        case 'days':
          // For daily frequency, we can just use the date
          key = date.toISOString().split('T')[0];
          break;
        case 'weeks':
          // Get the start of the week (Sunday) and adjust by frequency value
          ...
          break;
        case 'months':
          // Calculate which month period this falls into
          ...
          break;
        case 'years':
          // Calculate which year period this falls into
  ...
  const formatChartData = (transactions) => {
    ...
    const aggregatedData = aggregateTransactions(transactions, incomeFrequency);
    ...
    return aggregatedData.map(item => ({
      ...
      date: new Date(item.date).toLocaleDateString('en-US', { 
        month: 'short', 
        day: 'numeric',
        ...(incomeFrequency?.unit === 'year' && { year: 'numeric' })
      })
    }));
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[1000px]">
        <DialogHeader>
          <DialogTitle>{incomeName || 'Income'} Transactions</DialogTitle>
        </DialogHeader>
        ...
        {transactions && transactions.length === 0 && (
          <div className="py-4 text-center text-gray-500">No Transactions are associated with this Income.</div>
        )}
                          ...
                          <>
                            ...
                            {index === 1 && (
                              <div className="mt-1.5 flex basis-full items-center border-t pt-1.5 text-xs font-medium text-foreground">
                                Difference
                                <div className="ml-auto flex items-baseline gap-0.5 font-mono font-medium tabular-nums text-foreground">
                                  <span className={item.payload.transactionAmount - item.payload.incomeAmount < 0 ? 'text-red-500' : 'text-green-500'}>
                                    <span className="font-normal">
                                      {(item.payload.transactionAmount - item.payload.incomeAmount < 0 ? '-' : '+')}
                                      $
                                    </span>
                                    {Math.abs(item.payload.transactionAmount - item.payload.incomeAmount).toFixed(2)}
                                  </span>
                                </div>
                              </div>
                            )}
