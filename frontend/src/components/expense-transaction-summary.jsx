...
export default function ExpenseTransactionSummary({ isOpen, onOpenChange, expenseId, expenseName, expenseAmount, expenseFrequency }) {
  ...
  const formatChartData = (transactions) => {
    ...
    const aggregatedData = aggregateTransactions(transactions, expenseFrequency);
    ...
    return aggregatedData.map(item => ({
      ...
      date: new Date(item.date).toLocaleDateString('en-US', { 
        month: 'short', 
        day: 'numeric',
        ...(expenseFrequency?.unit === 'year' && { year: 'numeric' })
      })
    }));
  };
                          ...
                          <>
                            ...
                            {index === 1 && (
                              <div className="mt-1.5 flex basis-full items-center border-t pt-1.5 text-xs font-medium text-foreground">
                                Difference
                                <div className="ml-auto flex items-baseline gap-0.5 font-mono font-medium tabular-nums text-foreground">
                                  <span className={item.payload.transactionAmount - item.payload.expenseAmount < 0 ? 'text-red-500' : 'text-green-500'}>
                                    <span className="font-normal">
                                      {(item.payload.transactionAmount - item.payload.expenseAmount < 0 ? '-' : '+')}
                                      $
                                    </span>
                                    {Math.abs(item.payload.transactionAmount - item.payload.expenseAmount).toFixed(2)}
                                  </span>
                                </div>
                              </div>
                            )}
